...
One span per stage, nested under a root span for the extraction.
...
| `ovrin.validate` | rule evaluation |
| `ovrin.ground` | value-to-source matching |
| `ovrin.score` | confidence scoring |
...
`outcome` is `ok`, `invalid`, `review` or `error`. `reason` on `ovrin.reviews`
is the `ReviewReason` cause — `low_confidence`, `disagreement`,
`missing_required`, `ungrounded`, `cross_field`, `suspicious` — never a field
name.
