...
> **Nothing has been measured yet.** Twenty-five documents are committed and
> the harness runs against them, but every one is synthetic and no run has been
> committed: `eval/report/` holds a no-run report whose figures are zeros
> produced by the renderer over an empty result set. Every number in this
...
eval/
  corpus/
    invoices/
      ...
      001.expected.json
      001.meta.yaml
    receipts/  forms/  statements/  identity/
  corpusgen/                    the generator for the synthetic documents
  schema/
    invoice.go  receipt.go  form.go  statement.go  identity.go
  report/
    no-run.json                 the placeholder; no run has been committed
    no-run.md
  eval_test.go
```
...
go test -tags=eval ./eval/... -baseline report/no-run.json
```
...
Committed to `eval/report/`, so a regression shows up in a diff. The only file
there today is `no-run.json` and its `no-run.md` rendering, regenerated by
`make report`: a report shaped exactly like a real one, over zero results, so
that the format is reviewable before the first paid run and so that nothing in
the repository can quietly cite an empty file as a measurement.

A committed run looks like this. The figures below are illustrative — this is
the shape, not a result:
...
All zeros because nothing has been run, and `n=40` because a real corpus is
larger than the twenty-five synthetic documents committed today. **Reports are
committed even when they are bad** — a public record of how wrong we are is the
only kind that stays honest ([`rules.md` §9.5](rules.md#9-documentation)).
