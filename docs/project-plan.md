...
**Released at v1.0.0. Unmeasured.**

| Artefact | State |
|---|---|
| Architecture decision records | **32**, all Accepted |
...
| Public API surface | `api/ovrin.txt`, generated, **364** entries — a compatibility contract since v1.0.0 ([ADR-0032](adr/0032-v1-is-an-api-promise.md)) |
...
| Release tags | **All nine modules at `v1.0.0`** (2026-09-12). The first release was the core at `v0.3.0` (2026-09-05). No module carries a `replace` directive |
...
| An outside production deployment | Somebody outside the project running ovrin and reporting back | — |

These three are ADR-0024's evidence conditions. They stopped gating the version
number when v1.0.0 shipped ([ADR-0032](adr/0032-v1-is-an-api-promise.md)), and
they are still what stands between the library and an honest accuracy claim.
The first two wait on real documents; the third waits on someone outside the
project, which is why it is the hardest to plan.
...
**Done — cut the releases.** The core was tagged `v0.3.0` on 2026-09-05 with
the adapters at `<path>/v0.1.0`, and all nine modules were tagged `v1.0.0` on
2026-09-12. Every `replace` directive is gone
([`RELEASING.md`](../RELEASING.md)).

**1. Get real documents into the corpus.**
...
**2. Run the harness and commit the report.**
...
**3. Calibrate the confidence weights.**
...
**4. Find a production deployment that is not the maintainer's.**
...
**5. Publish per-stage benchmarks.**
...
| **Corpus stays synthetic because redistributable real documents are hard to source** | high | high | Every accuracy claim stays unmakeable, whatever the version number says. The synthetic half is done and is what a regression test needs; public government forms are the next cheapest source. Accept that the corpus will be unrepresentative before it is representative. |
| **Confidence weights never get calibrated** | medium | high | No accuracy claim can lean on confidence. It stays documented as a ranking signal — which is honest, and much less useful. |
